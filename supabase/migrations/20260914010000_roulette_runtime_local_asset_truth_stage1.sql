-- Roulette corrective Stage 1: local stem rows are scoped to a stable desktop
-- installation identity. Existing rows remain as nullable legacy rows and are
-- adopted only after the current installation proves the local file exists.

alter table public.roulette_stem_assets
  add column if not exists installation_id text;

alter table public.roulette_stem_assets
  drop constraint if exists roulette_stem_assets_track_type_unique;
...
begin
  if not exists (
    select 1
      from pg_constraint
     where conrelid = 'public.roulette_stem_assets'::regclass
       and conname = 'roulette_stem_assets_track_type_installation_unique'
  ) then
    alter table public.roulette_stem_assets
      add constraint roulette_stem_assets_track_type_installation_unique
      unique (track_id, stem_type, installation_id);
  end if;
end
$$;

create index if not exists roulette_stem_assets_installation_status_idx
  on public.roulette_stem_assets (installation_id, status, stem_type);

-- Replace the old unscoped pair-commit overload so all new ready rows are
-- explicitly owned by the installation that published the local files.
