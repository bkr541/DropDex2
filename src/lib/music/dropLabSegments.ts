...
export function resolveTrackDurationMs(track: RekordboxTrack, beats: BeatEntry[] = []): TrackTiming {
  if (track.duration_seconds != null && track.duration_seconds > 0) {
    return { durationMs: track.duration_seconds * 1000, usedDurationSource: 'track' };
  }
...
function shiftByBeats(ms: number, beats: BeatEntry[], beatOffset: DropLabBeatOffset, fallbackBpm: number | null): number {
...
function bpmWindowMs(bpm: number | null, barCount: DropLabBarCount): number | null {
  if (bpm == null || bpm <= 0) return null;
  return (60_000 / bpm) * 4 * barCount;
}

function segment(startMs: number, endMs: number, durationMs: number | null, timingSource: 'beat-grid' | 'bpm'): DropLabTimeSegment | null {
  ...
  return { startMs: safeStart, endMs: safeEnd, durationMs: safeEnd - safeStart, timingSource };
}
...
export function buildDropLabSegments(input: {
  ...
  const sourceDuration = resolveTrackDurationMs(input.sourceTrack, input.sourceBeats).durationMs;
  const candidateDuration = resolveTrackDurationMs(input.candidateTrack, input.candidateBeats).durationMs;
  ...
  const sourceDropIndex = findDownbeatIndexAtOrBefore(sourceDownbeats, input.sourceDrop.dropMs);
  if (sourceDropIndex >= 0) {
    const start = sourceDownbeats[Math.max(0, sourceDropIndex - input.barCount)]?.ms ?? 0;
    sourceSegment = segment(start, input.sourceDrop.dropMs, sourceDuration, 'beat-grid');
  } else {
    const windowMs = bpmWindowMs(input.sourceTrack.bpm, input.barCount);
    if (windowMs != null) {
      sourceSegment = segment(input.sourceDrop.dropMs - windowMs, input.sourceDrop.dropMs, sourceDuration, 'bpm');
    }
  }
  ...
  const candidateDropIndex = candidateDownbeats.findIndex((beat) => beat.ms >= alignedCandidateDropMs);
  if (candidateDropIndex >= 0) {
    const end = candidateDownbeats[candidateDropIndex + input.barCount]?.ms ?? candidateDuration ?? alignedCandidateDropMs;
    candidateSegment = segment(alignedCandidateDropMs, end, candidateDuration, 'beat-grid');
  } else {
    const windowMs = bpmWindowMs(input.candidateTrack.bpm, input.barCount);
    if (windowMs != null) {
      candidateSegment = segment(alignedCandidateDropMs, alignedCandidateDropMs + windowMs, candidateDuration, 'bpm');
    }
  }
