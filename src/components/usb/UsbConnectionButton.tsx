import { Usb, Unplug, Loader2, AlertTriangle, XCircle, WifiOff } from 'lucide-react';
...
function StatusDot({ status }: { status: UsbStatus }) {
  ...
  if (status === 'permission-required' || status === 'unavailable') {
...
function StatusIcon({ status, size = 18 }: { status: UsbStatus; size?: number }) {
  ...
  if (status === 'connected') return <Usb size={size} />;
...
function statusTitle(status: UsbStatus, volumeName: string | null): string {
  switch (status) {
    ...
    case 'unavailable': return 'USB drive not found — reconnect the drive';
...
export function UsbConnectionButton({ collapsed = false }: UsbConnectionButtonProps) {
  const { status, volumeName, error, structureWarning, connect, disconnect, reconnect, ensurePermission } = useUsbConnection();
  ...
  function handleClick() {
    ...
    if (status === 'connected') return; // click handled by disconnect button only
