...
## Cue Apply Stage 11: DJcues parity and `DjmdCue` support policy

DropDex keeps one cue truth pipeline: imported evidence → canonical editor cues → saved draft → guarded local preflight → staging writer → reopened verification. Stage 11 does not make the editor or cloud draft authoritative for current local Rekordbox truth.

### Auto Cue A-H contract

The production `autoCueStrategy.ts` contract matches the working DJcues A-H constants. Hot colors are Rekordbox `ColorTableIndex`; Memory colors are the separate local `DjmdCue.Color` encoding.

| Slot | Hot semantic | Hot kind | Hot `ColorTableIndex` | Paired Memory | Memory `Color` | Memory `ColorTableIndex` | Active by default |
|---|---|---:|---:|---|---:|---:|---|
| A | First Beat | 1 | 18 | point at A | 4 (Green) | null | n/a |
| B | Loop In | 2 | 18 | loop at B | 4 (Green) | 0 | no |
| C | Vocal / Buildup | 3 | 32 | point 16 bars before, clamped to first valid beat | 3 (Yellow) | null | n/a |
| D | Drop | 5 | 42 | point 16 bars before, clamped to first valid beat | 1 (Red) | null | n/a |
| E | Breakdown | 6 | 1 | point 16 bars before, clamped to first valid beat | 6 (Blue) | null | n/a |
| F | Special / energy recovery | 7 | 56 | point 16 bars before, clamped to first valid beat | 7 (Purple) | null | n/a |
| G | Outro | 8 | 9 | point 16 bars before, clamped to first valid beat | 5 (Aqua) | null | n/a |
| H | Loop Out | 9 | 0 | loop at H | 2 (Orange) | 0 | no |

Generated pairs persist `pairedHotCueSlot` plus their per-pair Memory offset. Moving a generated Hot Cue moves only its generated Memory pair; C-G clamp at the first valid beat and B/H paired loops retain the same loop range. Manual Memory Cues are not pair-owned and are not deleted or moved by those operations.

### Current `DjmdCue` field inventory

The bridge inventories every column exposed by the current pyrekordbox 0.4.4 `DjmdCue` inventory targeted by this stage, including inherited `StatsFull` columns. Runtime mutation validates the installed model column set and fails closed if a future/unrecognized column appears or an expected column disappears.

| Classification | Fields | Policy |
|---|---|---|
| Editable semantic | `InMsec`, `OutMsec`, `Kind`, `Color`, `ColorTableIndex`, `ActiveLoop`, `Comment` | Owned by the canonical desired cue and verified after staged reopen. |
| Preserved semantic/opaque | `InFrame`, `InMpegFrame`, `InMpegAbs`, `OutFrame`, `OutMpegFrame`, `OutMpegAbs`, `BeatLoopSize`, `CueMicrosec`, `InPointSeekInfo`, `OutPointSeekInfo` | Preserve current-local values when the same cue can be proven at unchanged timing. Non-default values block an unsafe timing rewrite or ambiguous complete-set replacement. |
| Writer-derived/regenerated | `ID`, `ContentID`, `ContentUUID`, `UUID`, `rb_data_status`, `rb_local_data_status`, `rb_local_deleted`, `rb_local_synced`, `usn`, `rb_local_usn`, `created_at`, `updated_at` | Row identity, target identity, and database lifecycle/stat fields are derived from the verified target or regenerated by the writer/database layer. |

For replacement rows with no source precision/seek semantics, DropDex keeps the established DJcues-compatible defaults (`InFrame=0`, MPEG in-fields `0`, point out-fields `-1`, loop out-fields `0`, `BeatLoopSize=0`, `CueMicrosec=0`, seek-info `null`). These defaults are not treated as proof that arbitrary non-default source values are safe to discard.

A preserved field may rebind after a prior successful complete-set replacement only when the current local row has a unique matching Rekordbox Kind and unchanged timing. This supports Stage 10 rebase/edit/apply cycles even though replacement row IDs are regenerated. Ambiguous ownership fails closed.

### Round-trip guarantee

The supported guarantee is **semantic round-trip, not byte-for-byte database round-trip**. DropDex verifies editable cue semantics plus every preserved precision/seek field after reopening the staging database. Primary keys/UUIDs and Rekordbox row lifecycle/stat metadata may legitimately change. A cue with non-default opaque timing/seek metadata cannot be moved unless DropDex can safely regenerate that metadata; today such a move is intentionally blocked rather than guessed.
